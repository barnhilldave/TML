...
#' x0 <- c(0,0,0)
#' VE.HAR(D_s, x0, I = 50)
#' VE.HAR(D_s, x0, I = 50,add='min')

VE.HAR <- function(D_s, x0, I = 1,add='max'){
  k<-ncol(D_s)-1
  ...
  for(i in 1:I){
    ...
    if(nrow(bps)>2){
      ...
      while (t <nrow(bp)){
        t=t+1
        dists<-as.vector(apply(D,1,function(x) trop.dist.hyp(-x,bp[t,],'min'))) # Calculates distance of bend point to all vertices
        if(all(dists>1e-8)){
          l<-rbind(l,bp[t,])
        }
        else{
  ...
  if(add=='min'){
    for(i in 1:I){
      ...
      if(k == 1) # If subset k=1 then this is just a vertex
        ...
        u <- project.pi(U, x1,add='min') # If k>1 project x0 onto the polytope
      }                      # defined by the subset of vertices
      if(k == (d[1] - 1)) # If the cardinality of U is e-1
        v <- V            # the complement is just the leftover vertex
      else
        v <- project.pi(V, x1,add='min') # Otherwise it is a projection
      Gam<-unique(TLineSeg(v,u,add='min')) # Get unique bendpoints in a tropical line
      bps<-normaliz.polytope(matrix(unlist(Gam),length(Gam),d[2],byrow=TRUE)) # Normalize the bendpoints
      # We calculate the tropical distance of the bendpoints (not the end points) of the tropical
      # line segment to each boundary vertex defining a hyperplane.
      # If there is more than one bendpoint, calculate the distance of each one.  If not then just
      ...
      if(nrow(bps)>2){
        ...
        while (t <nrow(bp)){
          t=t+1
          dists<-as.vector(apply(D,1,function(x) trop.dist.hyp(-x,bp[t,],'max'))) # Calculates distance of bend point to all vertices
          if(all(dists>1e-8)){
            l<-rbind(l,bp[t,])
          }
          else{
        ...
        x<-HAR.TLineSeg(l[1,], l[nrow(l),],add='min') # Conduct HAR on line segment between u and the last bendpoint.
        x<-normaliz.vector(x)
      }
      else{
        x<-HAR.TLineSeg(u,v,add='min') # Conduct HAR if there are no bendpoints.
        x<-normaliz.vector(x)
      }
    }
  }
